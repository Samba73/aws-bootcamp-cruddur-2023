...
### Data Modelling a Direct Messaging System using Single Table Design
 For the direct messaging in Cruddur, no relational option of DynamoDB was proposed. It is mostly approach with single table design also called flat table design. This is more flexible than and regular RDS in that all the data are captured in same table which eliminates the concept of joins.

1) Direct messaging establishes conversation between 2 users. so there should be one common group created for both users to continue the conversation. This is `message_groups` for both user.
2) Under this selected `message_group`, individual `messages' from both users will be displayed
3) Any of the user who want to respond cant initiate a `message` and send it to other user

 ### For Cruddur direct messaging 4 patterns are 
 1) Pattern A - Message Groups :  `message_groups`
 2) Pattern B - Messages: List of messages: `messages'
 3) Pattern C - Message: Respond to previous conversation: `message`
 4) Pattern D - Update Message Groups `message_groups` when the `message` is created
...
aws dynamodb scan  --table-name $1 $ENDPOINT_URL
```
...
print('user_handle is', message_user)
...
def get_users():
    print(sys.argv[1])
    message_user = sys.argv[1]
    query = extract_query('messages', 'user')
    users_int = query_execution_array(query, {
        "my_handle": message_user
    })
    message_user  = next((item for item in users_int if item['handle'] == message_user), None)
    print(message_user)
    results = {
        "my_handle": message_user
    }
...
tableName = "cruddur-message"
message_group_id = "5ae290ed-55d1-47a0-bc6d-fe2bc2700399" 
uuid = users['my_handle']['uuid']
#date_str = '2023-03-25T11:34:26.233405+00:00'
partition_key_value = {'S': f"GRP#{uuid}"}
sort_key_prefix = {'S': str(datetime.datetime.now().year)}
print('parition key value', partition_key_value)
query_string = {
    'TableName': tableName,
    'ScanIndexForward': False,
...
for item in items:
    ...
    strippedtime        = datetime.datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S.%f%z")
    formatted_datetime  = strippedtime.strftime('%d-%m-%Y %I:%M %p')
    print(f"Handle             Message                    Timestamp              ")
    print(f"{user_handle: <12} {message[:30]} {formatted_datetime: >20}")
```
...
DynamoDb stream is enabled and lambda function created which will be triggered for updating the message_groups when there is message response
...
### Rapid Data Modelling and implementation of DynamoDB with DDB Local
...
```
4) Seed the table
Same as above in point `Rapid Data Modelling and implementation of DynamoDB with DDB Local`

### db.py for RDS implementation
Going through video had doubts about class and its implementation so I preferred to do as inidividual functions rather than as methods of class

### Adding new `Message Group` through url was not implemented, instead I added New Message button (`MessageGroupFeed` component) and managed the new message groups creation through this react component
